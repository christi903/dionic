...
import { PhDCourse } from '../../data/scholarships';
import SpecializationsModal from './SpecializationsModal';

interface PhDCourseCardProps {
  course: PhDCourse;
}

const PhDCourseCard: React.FC<PhDCourseCardProps> = ({ course }) => {
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-gray-800">Research Areas:</span>
              </div>
              <button
                onClick={handleViewAllSpecializations}
                className="text-amber-600 hover:text-amber-700 text-xs font-medium px-3 py-1 rounded-full border border-amber-200 hover:border-amber-300 transition-colors duration-200"
              >
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {course.specializations.length > 3 && (
                <button
                  onClick={handleViewAllSpecializations}
                  className="bg-amber-600 hover:bg-amber-700 text-white text-xs px-3 py-1 rounded-full transition-colors duration-200"
                >
