...
import {
  additionalMedicalSupplies,
  advancedMedicalEquipment,
} from "../data/remainingCategories";
...
const MedicalSupplyPage = () => {
  ...
  }>({ open: false, productName: "", productId: null });
  ...
  // Number of background slides
  const slideCount = 4;

  // Auto-rotate background images every 4 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prevIndex) => (prevIndex + 1) % slideCount);
    }, 4000);
  ...
  const services = [
    {
      ...
      description:
        "All our medical equipment meets international quality standards and certifications.",
    },
    {
      icon: <Heart className="h-8 w-8" />,
      title: "Expert Support",
      description:
        "24/7 technical support and maintenance services for all equipment.",
    },
    {
      icon: <Activity className="h-8 w-8" />,
      title: "Training & Installation",
      description:
        "Complete installation and staff training for optimal equipment utilization.",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative text-white py-16 overflow-hidden">
        ...
        <div className="absolute inset-0">
          {Array.from({ length: slideCount }).map(
            (_: unknown, index: number) => (
              <div
                key={index}
                className={`absolute inset-0 bg-slide-image bg-slide-${index} ${index === currentImageIndex ? "active" : "inactive"} slide-image`}
                aria-hidden="true"
              />
            ),
          )}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <motion.h1
              className="text-4xl sm:text-5xl font-bold mb-6 text-black drop-shadow-lg"
            ...
            <motion.p
              className="text-xl text-black max-w-3xl mx-auto leading-relaxed drop-shadow-md"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 0.6, delay: 0.2 }}
            >
              Premium medical instruments, equipment, and supplies for
              healthcare facilities across Tanzania. Quality you can trust,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">
              We provide comprehensive support to ensure your medical equipment
      ...
      <QuantityModal
        ...
        onClose={() =>
          setQuantityModal({ open: false, productName: "", productId: null })
        }
        onConfirm={(q) => {
          const product = allProducts.find(
            (p) => p.id === quantityModal.productId,
          );
